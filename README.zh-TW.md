...
**視窗開得早，一個工作日能裝下更多個。** 9 點上班馬上開始用，視窗就是 9:00-14:00 和 14:00-19:00，到下班為止只有兩個。如果 6:00 已經自動開好一個，這一天就由 6:00-11:00、11:00-16:00 和 16:00-21:00 涵蓋 —— 同樣的在座時間，三個額度。下面的固定時刻就是用來釘住這個位置的。
...
開一個視窗要花掉一次請求，所以 Claude Plus 把它壓到最小：用該助手最便宜的模型，不帶工具，也不寫入歷史。模型會改朝換代，一旦助手不再認得目前用的這個，它會明確報錯 —— Claude Plus 隨即改用該助手自己的預設模型並通知你一次，而不是一個視窗一個視窗地失敗下去，等人去翻紀錄。

```bash
~/.claude/claude-plus/claude-plus.sh model                 # 查看每個助手用什麼模型保溫
~/.claude/claude-plus/claude-plus.sh model claude sonnet   # 自己指定一個
~/.claude/claude-plus/claude-plus.sh model claude auto     # 恢復成最便宜的那個
```

## 提醒

Claude Plus 平時不出聲，只在需要你處理時才響，而且只報告五件事：
...
| **恢復正常** | 從上面任一故障中恢復了 |
| **保溫模型失效** | 某個助手原本用來保溫的模型已經不存在了，之後改用它自己的預設模型 |
