...
安裝指令碼會包裝你現有的 status line（顯示和原本完全一樣），並加入自己的 hook，不動其他工具的 hook。`settings.json` 會先備份；重複執行安裝指令碼是一次乾淨的升級，而不是裝第二份。升級會保留正在進行中的一切：等待續跑的工作階段、下一次排好的工作，以及你的通知指令碼。
...
```bash
~/.claude/claude-plus/claude-plus.sh status   # 排程器在不在跑、排了什麼、有誰在等、認證還有效嗎
...
| **連續失敗** | 接連幾次保溫都失敗了 |
| **排程停了** | 排好的工作過了很久還沒執行，視窗不會被保溫、工作階段也不會續跑；通常是 `atd` 沒在執行 |
| **恢復正常** | 從上面任一情況中恢復了 |
